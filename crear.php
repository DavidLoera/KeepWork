<?php
    ...
    $db = new Database();
        ...
        $sql = "SELECT cve_egresado, b.cve_usuarios, b.username, nombrec, genero, estado_ci, pais FROM egresados as a,users as b WHERE b.username =:usuario";
        ...
        if(isset($_POST['cve_egresado']) ){
         ...
         $consulta = "INSERT into curriculum (cve_egresado, nombreegresado, genero, telefono, email, pais, domicilio, ciudad, nombreuniver, carrera, perfilpro, description) VALUES(:cve_egresado , :nombreegresado, :genero, :telefono, :email, :pais, :domicilio, :ciudad, :nombreuniver, :carrera, :perfilpro, :description)";
         $query = $db->connect()->prepare($consulta);

         if( $query->execute(['cve_egresado' => $egresado, 'nombreegresado' => $nombrec, 'genero' => $genero, 'telefono' => $telefono, 'email' => $email, 'pais' => $pais,'domicilio' => $domicilio, 'ciudad'=>$ciudad, 'nombreuniver' => $universidad, 'carrera' => $carrera, 'perfilpro' => $perfilpro, 'description' => $description])){
      ...
      <form method="POST" action="">
       <input name="description" type="hidden" value="<?= $usuarion ?>">
        <input name="cve_egresado" type="hidden" value="<?= $egresado->cve_egresado; ?>">
            ...
            <input type="text" name="domicilio" id="username" placeholder="Escriba su domiciclio"  require>
            ...
            <input type="text" name="universidad" id="username" placeholder="Nombre de la universidad" required>

            <label for="nombre" class="colocar_nombre">Carrera *
            <span class="obligatorio">*</span>
            </label>
            <input type="text" name="carrera" id="username" placeholder="Nombre de la carrera" required>

            <label for="nombre" class="colocar_nombre">Perfil profesional *
            <span class="obligatorio">*</span>
            </label>
            <textarea name="perfilpro" rows="10" cols="48" style="border-color: #f3cf47;" placeholder="Breve descripción de tu perfil profesional"></textarea>
